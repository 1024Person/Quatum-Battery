...
def claculate(triangle):
    ...
    H_int_ =J*hbar*(np.dot(pauli_x_1,pauli_x_3)+np.dot(pauli_y_1,pauli_y_3)+triangle*np.dot(pauli_z_1,pauli_z_3)) +\
          J*hbar*(np.dot(pauli_x_2,pauli_x_3)+np.dot(pauli_y_2,pauli_y_3)+triangle*np.dot(pauli_z_2,pauli_z_3))
    H = H_ch +H_int + H_int_
    # 定义功率算符
    P_opea = 1/1j*(np.dot(H0+H_ch,H_int_)-np.dot(H_int_,H0+H_ch))   # 这个无论怎么看都是0 
