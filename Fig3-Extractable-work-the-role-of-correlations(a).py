...
def calc_ratio(n):
    ...
    for i in range(n):
        ...
        op_list[i] = s_up
        temp = tensor(op_list)
        s_up_list.append(temp)
        # 构造σ-算符
        op_list[i] = s_down
        temp = tensor(op_list)
        s_down_list.append(temp)
        # 构造fork态
    ...
    # 直积构造量子电池fork态
    psi_f = tensor(psi_f)
    # 获取N个单位矩阵的直积，下面构造缠身湮灭算符的时候需要用到
    mat_tensor = tensor(op_list)
    # 构造产生湮灭算符
    a = tensor(mat_tensor, destroy(M))
    a_dag = a.dag()
    # 构造哈密顿量
    HA = omega0 * a_dag * a
    HB = 0
    ...
    W1 = E1 - passive_e(HB1, psi_1B)
    E2 = expect(HB, psi_2) # 相干态
    W2 = E2 - passive_e(HB1, psi_2B)
    E3 = expect(HB, psi_3) # 压缩态
    W3 = E3 - passive_e(HB1, psi_3B)
